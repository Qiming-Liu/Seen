...
public class SettingActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (pref.getBoolean("infoOK", false)) {
            ...
            if (pref.getBoolean("imageExist", false)) {
                ...
                System.out.println("[ProSS]本地头像存在");
                UserInfo.headImage = pref.getString("headImage", "");//读取图片路径
                if (UserInfo.headImage != null) {

                    System.out.println("[ProSS]显示头像");
                    Bitmap bitmap = BitmapAndStringUtils.StringToBitmap(UserInfo.headImage);
                    iv_personal_icon.setImageBitmap(bitmap);
                }

            } else {
                System.out.println("[ProSS]本地头像不存在");
    ...
    public void exit(View view) {
        ...
        editor.putBoolean("infoOK", false);
            ...
            queue.add(0, request, new OnResponseListener<String>() {
                ...
                public void onSucceed(int what, Response<String> response) {
                    if (response.responseCode() == 200) {
                        ...
                        if(!UserInfo.userID.equals("")) editor.putBoolean("loginSuccess", true);

                        if(!UserInfo.nickname.equals("")) editor.putBoolean("infoOK", true);
                        editor.commit();
